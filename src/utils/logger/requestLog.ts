...
export function logDetails(request: IRequestModel, reqHeaders: {}, requestBody: any, responseStatus: number, responseHeaders: ITableData[], responseData: any) {
	logRequestDeatils(request, reqHeaders, requestBody);
	if (getLogOption()) {
		logResponseDeatils(responseStatus, responseHeaders, responseData);
	}
}
...
function logResponseDeatils(status: number, headers: ITableData[], responseData: any) {
	try {
