...
function renderPreFetchStep(step: ExportPreFetchStep, indent: string): string {
  const testsXml = step.tests.length > 0
    ? `\n${indent}  <tests>\n${step.tests
      .map(t => `${indent}    <test name="${escapeXml(t.name)}" result="${t.passed ? 'Pass' : 'Fail'}" actualValue="${escapeXml(t.actualValue ?? '')}" />`)
      .join('\n')}\n${indent}  </tests>`
    : '';

  const childrenXml = step.children.length > 0
...
function renderRequest(r: ExportRequestResult): string {
  ...
  const testsXml = r.tests.length > 0
    ? `\n    <tests>\n${r.tests
      .map(t => `      <test name="${escapeXml(t.name)}" result="${t.passed ? 'Pass' : 'Fail'}" actualValue="${escapeXml(t.actualValue ?? '')}" />`)
      .join('\n')}\n    </tests>`
    : '';
...
export function toXml(report: ExportReport): string {
  const { context, summary } = report;
  const requestsXml = report.results.map(renderRequest).join('\n');

  return `<?xml version="1.0" encoding="UTF-8"?>
<fetchClientReport scope="${escapeXml(context.scope)}" name="${escapeXml(context.name)}" generatedAt="${escapeXml(summary.generatedAt)}">
  <summary totalRequests="${summary.totalRequests}" passedRequests="${summary.passedRequests}" failedRequests="${summary.failedRequests}" totalDurationMs="${summary.totalDurationMs}" totalTests="${summary.totalTests}" passedTests="${summary.passedTests}" failedTests="${summary.failedTests}" />
${requestsXml}
</fetchClientReport>
`;
}
