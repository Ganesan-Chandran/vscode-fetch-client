...
function buildSummary(results: RunResult[]): ExportSummary {
  ...
  const passedRequests = results.filter(r => !r.isError && r.status >= 200 && r.status < 400).length;
...
export function buildExportReport(results: RunResult[], context: ExportContext): ExportReport {
  return {
    context,
