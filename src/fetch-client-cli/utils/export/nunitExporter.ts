...
export function toNUnit(report: ExportReport): string {
  ...
  const testCases = results.map((r, idx) => renderTestCase(r, idx + 1)).join('\n');

  const total = results.length;
  const passed = results.filter(r => testCaseResult(r) === 'Passed').length;
  const failed = total - passed;
  const totalDurationSeconds = (results.reduce((sum, r) => sum + r.durationMs, 0) / 1000).toFixed(3);
