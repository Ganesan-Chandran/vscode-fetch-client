...
export function getCliConfig(): ICliConfig {
	...
	if (process.env.FC_DB_PATH) {
		...
		cliConfig.encryptionEnabled =
			process.env.FC_ENCRYPTION_ENABLED?.toLowerCase() === "true";
		if (cliConfig.encryptionEnabled && !process.env.FC_ENCRYPTION_KEY) {
			wrtieConsleError(
				"An encryption key is required when encryption is enabled. If you are running the CLI standalone, configure encryption using environment variables.",
			);
			process.exit(1);
		}
...
function defaultStoragePath(): string {
	...
	return join(homedir(), ".config", "Code", "User", "globalStorage", EXT_ID);
}
