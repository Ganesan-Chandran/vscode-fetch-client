...
export async function executeSingleRequest(
	...
	const { request, collection, folderId, variable, effectiveVarId } = context;
...
export async function executeCollection(
	...
	for (const context of contexts) {
		const { collection, leaves, requestMap, variable, effectiveVarId } =
			context;

		if (leaves.length === 0) {
			writeConsoleLog(`Collection '${collection.name}' is empty, skipping.`);
			continue;
		}

		printSection(`Collection: ${collection.name} (${leaves.length} requests)`);
...
export async function executeFolder(
	...
	const { folder, collection, leaves, requestMap, variable, effectiveVarId } =
		context;
