...
const MainUI = () => {
	...
	useEffect(() => {
		reqSettingsRef.current = reqSettings;
	}, [reqSettings]);
	...
	useEffect(() => {
		if (
			(response.status !== 0 || response.isError) &&
			resClass === "zero-height"
		) {
			setOpenPanel(1);
		}
	}, [response]);
	...
	useEffect(() => {
		...
		if (reqId !== "undefined" && isRunItem === "undefined") {
			vscode.postMessage({
				type: requestTypes.openExistingItemRequest,
				data: reqId,
			});
		} else if (reqId !== "undefined" && isRunItem === "OpenAndRun") {
			dispatch(UIActions.SetRunItemAction(true));
			vscode.postMessage({
				type: requestTypes.getOpenAndRunItemDataRequest,
				data: reqId,
			});
		...
		dispatch(
			Actions.SetReqColDetailsAction(
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			vscode.postMessage({
				...
				data: {
					reqData,
					isNew,
					colId: colId === "undefined" ? undefined : colId,
				},
			});
		};
	...
	function setBothHalf() {
		...
		document
			.getElementById("fullscreen-expand-btn")
			?.classList.remove("fullscreen-btn-invisible");
	}
	...
	function setReqFull() {
		...
		document
			.getElementById("fullscreen-expand-btn")
			?.classList.add("fullscreen-btn-invisible");
	}
							...
							>
								<div>
									...
									<div
										className={
											saveVisible
												? "save-text save-visible save-text-horizontal"
												: "save-text save-invisible"
										}
									>
										Saved Successfully
									</div>
