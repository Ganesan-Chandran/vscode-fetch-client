...
export const ResponseSection = (props: any) => {
	...
	const { horizontalLayout, responseLimit } = useSelector(
		(state: IRootState) => state.uiData,
	);
	...
	const editor = useMemo(() => {
		return (
			<AceEditor
				...
				language={
					response.responseType?.format
						? response.responseType.format
						: responseType[1].value
				}
				readOnly={true}
	...
	function onDownloadFile() {
		vscode.postMessage({
			type: requestTypes.downloadFileTypeRequest,
	...
	function isPreViewVisible(): boolean {
		return (
			...
			response.responseType?.format === "html" ||
	...
	function getErrorSection() {
		return (
			<div className="res-not-support-panel">
				<div className="res-not-support-text error-text">
	...
	function getBinaryResponseSection() {
		return (
			<div className="res-not-support-panel">
				<div className="res-not-support-text">
				...
				{isPreViewVisible() && (
					<div className="toggle">
						...
						<input
							...
							name="sizeBy"
							value="dimensions"
							onChange={onJsonViewResponseClick}
							checked={viewType !== "raw"}
							id="sizeDimensions"
						/>
	...
	function menuSection() {
		return (
			<div className="word-wrap-button">
				<button
		...
		return (
			<div
				className={
					isPreViewVisible()
						? "response-editor"
						: "response-editor-without-preview"
				}
			>
				<div className={getEditorPanelCss()}>{editor}</div>
				<div className={getViewerPanelCss()}>
					{response.responseType?.format === "json" && (
						<JSONViewer data={response.responseData} />
					)}
					{response.responseType?.format === "html" && (
						<HTMLViewer data={response.responseData} />
					)}
					{response.responseType?.format === "xml" && (
						<XMLViewer data={response.responseData} />
					)}
				</div>
				{fullScreenMode ? (
	...
	return (
		<div className="response-content-panel">
			{response.isError ? (
				...
				<>
					<hr />
					{loading === true ? getLoadingSection() : getIdealSection()}
				</>
			) : response.responseData ? (
