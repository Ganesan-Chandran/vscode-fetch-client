...
const CurlUI = () => {
	...
	useEffect(() => {
		window.addEventListener("message", (event) => {
			if (event.data && event.data.type === responseTypes.runCurlResponse) {
				dispatch(
					ResponseActions.SetResponseCookiesAction(event.data.response.cookies),
				);
				dispatch(
					ResponseActions.SetResponseAction(event.data.response.response),
				);
				dispatch(
					ResponseActions.SetResponseHeadersAction(event.data.response.headers),
				);
			} else if (
				event.data &&
				...
				vscode.postMessage({
					...
					data: {
						...
						id: event.data.historyId,
						name: event.data.historyName,
						varId: event.data.varId,
					},
				});
			} else if (
				event.data &&
				event.data.type === responseTypes.curlErrorResponse
			) {
				setErrors({
					...errors,
					colSelect: "",
		...
		return (
			<div className="curl-options">
				<input
					...
					value="import"
					name="curl"
					checked={curlOption === "import"}
					onChange={() => onCurlOptionSelect("import")}
				/>{" "}
				Import
				<input
					...
					value="run"
					name="curl"
					checked={curlOption === "run"}
					onChange={() => onCurlOptionSelect("run")}
				/>{" "}
				Run (Without Save)
			</div>
		);
	...
	function onImportClick() {
		if (
			...
			vscode.postMessage({
				type: requestTypes.convertCurlToJsonRequest,
				data: {
					curl: curl,
					col: collection,
	...
	function getButtonSection() {
		return (
			<div
				className={
					curlOption === "run" ? "curl-run-btn-panel" : "curl-import-btn-panel"
				}
			>
				<button
					type="submit"
					className={
						curlOption === "run"
							? "submit-button curl-run-btn"
							: "submit-button curl-run-col-btn"
					}
					...
					<tbody>
						...
						{curlOption === "import" && (
							<tr>
								...
								<td className="col-2-size">{getButtonSection()}</td>
							</tr>
						)}
						{errors["curlError"] && (
							<tr>
								...
								<td className="col-2-size">
									<div className="error-text curl-error">
										{errors["curlError"]}
									</div>
								</td>
							</tr>
