...
const AutoRequest = () => {
	...
	const handleAutoRequestResponse = (data: any) => {
		...
		if (scoped) {
			setRows((current) =>
				current.map((row) => (row.colId ? row : { ...row, colId: data.colId })),
			);
	...
	useEffect(() => {
		const onMessage = (event: MessageEvent) => {
			...
			if (data.type === responseTypes.getAllCollectionNamesResponse) {
				...
				data.type === responseTypes.getCollectionsByIdWithPathResponse
			) {
	...
	const selectRequest = (index: number, value: string) => {
		...
		const name =
			requestLists[row.colId]?.find((request) => request.id === value)?.name ??
			"";
		updateRow(index, {
			reqId: value,
			reqName: name.split(";")[0] ?? "",
			parentId: name.split(";")[1] ?? "",
		});
	...
	const valid =
		...
		populatedRows.every(
			(row) =>
				row.colId &&
				row.reqId &&
				row.interval >= MIN_INTERVAL &&
				row.interval <= MAX_INTERVAL &&
				row.duration >= MIN_DURATION &&
				row.duration <= MAX_DURATION &&
				row.interval <= row.duration,
		);

	const activeScheduleIds = new Set(
		schedules
			.filter((item) => item.scheduleStatus === "running")
			.map((item) => item.scheduleId ?? item.id),
	);

	const scheduleGroups = Object.values(
		schedules.reduce<Record<string, IAutoRequestSchedule[]>>((groups, item) => {
	...
	const statusClass = (status: string) =>
		status === "success" || status === "completed"
			? "history-status history-status--ok"
	...
	return (
		<PanelLayout
			...
			footer={
				tab === "create" ? (
					<>
						{message && (
							<div className="reorder-status reorder-status--ok">{message}</div>
						)}
		...
		>
			<div className="autorequest-tabs">
				<button
					className={
						tab === "create" ? "autorequest-tab active" : "autorequest-tab"
					}
				...
				<button
					className={
						tab === "summary" ? "autorequest-tab active" : "autorequest-tab"
					}
					onClick={() => {
						setTab("summary");
						refresh();
			...
			{tab === "create" ? (
						...
						<>
							{isScheduleLimitReached && (
								<div className="autorequest-max-note">
									"A schedule is already running. It must finish, complete, or
									be stopped before you can create another."
								</div>
							)}
							{!isScheduleLimitReached && (
								<div className="autorequest-max-note">
							...
							>
								<div
									className={`autorequest-row autorequest-row--header ${!isGlobal ? "autorequest-row--scoped" : ""}`}
								>
									{isGlobal && (
								...
								{rows.map((row, index) => (
									<div
										className={`autorequest-row ${!isGlobal ? "autorequest-row--scoped" : ""}`}
										key={row.id}
									>
										{isGlobal && (
											<div className="autorequest-cell">
												<select
													...
													value={row.colId}
													onChange={(event) =>
														selectCollection(index, event.target.value)
													}
												>
										...
										<div className="autorequest-cell">
											<select
												...
												disabled={!row.colId}
												onChange={(event) =>
													selectRequest(index, event.target.value)
												}
											>
												{(
													requestLists[row.colId] ?? [
														{ id: "", name: "--select--" },
													]
												).map((request) => (
													<option
										...
										<div className="autorequest-cell">
											<input
												...
												disabled={!row.colId}
												onChange={(event) =>
													numeric(index, "interval", event.target.value)
												}
											/>
										</div>
										<div className="autorequest-cell">
											<input
												...
												disabled={!row.colId}
												onChange={(event) =>
													numeric(index, "duration", event.target.value)
												}
											/>
										</div>
										<div className="autorequest-cell">
				...
				<div className="autorequest-summary">
					<div className="summary-heading">
						<span>
							Schedules ({activeScheduleIds.size}/{MAX_ACTIVE_SCHEDULES} active)
						</span>
						<button className="submit-button" onClick={refresh}>
							Refresh
						</button>
					</div>
					<div className="autorequest-table schedule-table">
						...
						{sortedScheduleGroups.length ? (
							sortedScheduleGroups.map((group) => {
								...
								return (
									<div
										className={`autorequest-table-row ${
											selectedScheduleId ===
											(group[0].scheduleId ?? group[0].id)
												? "selected-row"
												: ""
										}`}
									...
									>
										<span>
											{(group[0].scheduleId ?? group[0].id).slice(0, 8)}
										</span>
										<span>{group.length}</span>
										<span>{collectionName(group[0].colId)}</span>
										<span className={statusClass(status)}>{status}</span>
										<span>
											{group.find((item) => item.scheduleStatus === "running")
												?.nextRunTime ?? "-"}
										</span>
					...
					<div className="history-section-title">Requests</div>
					<div className="autorequest-table request-table">
						<div className="autorequest-table-row header">
							<span>Schedule</span>
							<span>Request</span>
							<span>Status</span>
