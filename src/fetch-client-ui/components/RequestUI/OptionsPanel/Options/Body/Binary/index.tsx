...
export const Binary = () => {
	...
	const { body, headers } = useSelector(
		(state: IRootState) => state.requestData,
	);

	const [contentType, setContentType] = useState(
		body.binary.fileName ? getFileType(body.binary.fileName) : "",
	);
	...
	function updateContentType(contentTypeValue: string) {
		...
		let index = headers.findIndex(
			(item) =>
				item.isChecked &&
				item.key.trim().toLocaleLowerCase() === "content-type",
		);
		if (index === -1) {
			index = headers.findIndex(
				(item) => item.key.trim().toLocaleLowerCase() === "content-type",
			);
			if (index === -1) {
	...
	return (
		<div className="binary-panel">
			...
			<div className="file-upload-panel">
				<button className="file-upload-text" onClick={onSelectFile}>
					Select file
				</button>
				<div className="filename-text">{body.binary.fileName}</div>
				{body.binary.data && body.binary.data.length > 0 && (
					<div className="file-reset-panel">
						<button className="file-reset-text" onClick={onResetFile}>
							Reset File
						</button>
					</div>
				)}
			</div>
			{body.binary.fileName && body.binary.data.length === 0 && (
				<span className="file-not-available">
					{
						"The file is not available in the specified location. Please select the file again."
					}
				</span>
			)}
			{body.binary.data && body.binary.data.length > 0 && (
				<div className="content-type-panel">
					<span className="content-type-radio-button-text">
						Content Type :{" "}
					</span>
					<div className="content-type-radio-button-panel">
						<span className="content-type-radio-button-wrapper">
							<input
								className="content-type-radio-button"
								type="radio"
								id="manual"
								name="content-type"
								value="manual"
								onChange={onChanged}
								checked={body.binary.contentTypeOption === "manual"}
							/>
							{"Added by Manual"}
						</span>
						<span className="content-type-radio-button-wrapper">
							<input
								...
								onChange={onChanged}
								checked={
									body.binary.contentTypeOption === "application/octet-stream"
								}
							/>
						...
						{contentType && (
							<span className="content-type-radio-button-wrapper">
								<input
									...
									value={contentType}
									onChange={onChanged}
									checked={body.binary.contentTypeOption === contentType}
								/>
								{contentType}
							</span>
						)}
