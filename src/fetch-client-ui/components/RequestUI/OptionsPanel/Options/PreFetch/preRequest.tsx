...
export const PreRequest = (props: IPreRequestProps) => {
	...
	const { collectionList, colRequestList } = useSelector(
	...
	useEffect(() => {
		const handleMessage = (event: MessageEvent) => {
			if (
				...
				props.request.colId === event.data.colId
			) {
				...
				let parentId =
					col.reqs.length > 0
						? col.reqs.find((i) => i.id === reqId)?.name.split(";")[1]
						: "";
		...
		if (props.request.colId) {
			if (
				colRequestList?.findIndex((i) => i.id === props.request.colId) === -1
			) {
				vscode.postMessage({
					type: requestTypes.getCollectionsByIdWithPathRequest,
					data: props.request.colId,
				});
			} else {
				setSelectedRequestList(
					colRequestList.find((i) => i.id === props.request.colId)?.reqs ?? [],
				);
			}
		}
	...
	const onSelectCollection = (colId: string, index: number) => {
		...
		if (colRequestList?.findIndex((i) => i.id === colId) === -1) {
			...
			dispatch(
				Actions.SetSelectedReqAction(
					reqs.length > 0 ? reqs[0].id : "",
					index,
					reqs.length > 0 ? reqs[0].name.split(";")[1] : "",
				),
			);
		}
	};
	...
	const onSelectRequest = (
		...
		dispatch(
			Actions.SetSelectedReqAction(
				value,
				reqIndex,
				selectedRequestList[selectedIndex].name.split(";")[1],
			),
		);
	};
	...
	const addValue = (
		...
		localValue.condition[condIndex] = {
			...
			expectedValue:
				selectType === "expectedValue" ? value : condition.expectedValue,
			customParameter:
				selectType === "customParameter" ? value : condition.customParameter,
		};
	...
	function getActionList(row: ITest, reqIndex: number, conIndex: number) {
		...
		if (parameter) {
			actionList = row.parameter
				? row.parameter === "noCondition"
					? []
					: parameter["action"]
				: [{ name: "action", value: "" }];
		} else {
		...
		return (
			<div className="preReq-condition-block">
				...
				{conditions.map((item: ITest, conIndex: number) => {
					return (
						<div
							className="preReq-condition-panel"
							key={"preReq_cond_panel_" + props.reqIndex + conIndex}
						>
							...
							<div className="preReq-condition-param-panel">
								<TextEditor
									...
									onChange={(val) =>
										onSelectItem(val, reqIndex, conIndex, "expectedValue")
									}
							...
							<div className="preReq-condition-delete-panel">
								{conIndex !== conditions.length - 1 ? (
									<BinLogo
										...
										className="delete-button preReq-condition-delete-btn"
										onClick={() => onDelete(reqIndex, conIndex)}
									/>
								) : (
									<></>
								)}
			...
			<fieldset className={reqIndex !== 0 ? "preReq-field-panel" : ""}>
				...
				<div className="preReq-fields-row">
					...
					<div className="preReq-text-panel">
						...
						>
							{selectedRequestList?.length > 0 &&
								selectedRequestList.map(({ id, name }) => (
					...
					<div className="preReq-delete-panel">
						<BinLogo
							title="delete pre-request"
							className="delete-button"
							onClick={() => onDeleteReqClick(reqIndex)}
						/>
					</div>
				</div>
			</fieldset>
