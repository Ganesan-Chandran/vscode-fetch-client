...
export const RequestPanel = () => {
	...
	const { parentSettings, colId } = useSelector(
	...
	function addCookieHeader(currentCookie: ICookie) {
		...
		if (cookieHeader) {
			...
			let index = localTable.findIndex(
				(item) => item.key.toUpperCase() === "COOKIE",
			);
	...
	useEffect(() => {
		if (responseData?.cookies?.length > 0) {
			...
			vscode.postMessage({
				type: requestTypes.saveCookieRequest,
				data: cookie,
			});
		}
