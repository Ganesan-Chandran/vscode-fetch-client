...
export const HistoryBar = (props: IHistoryProps) => {
	...
	useEffect(() => {
		moreMenuWrapperRef.current = moreMenuWrapperRef.current.slice(
			0,
			history.length,
		);
	}, [history]);
	...
	function onDelete(
		...
		evt.stopPropagation();
		vscode.postMessage({
			type: requestTypes.deleteHistoryRequest,
			data: id,
			name: name,
		});
		setCurrentIndex(-1);
	...
	function openItem(id: string, name: string, isNewTab: boolean) {
		...
		vscode.postMessage({
			...
			data: { id: id, name: name, isNewTab: isNewTab },
		});
	}
	...
	function onRunClick(
		...
		vscode.postMessage({
			...
			data: { id: itemId, name: name, isNewTab: false },
		});
		setCurrentIndex(-1);
	}
		...
		return (
			...
			>
				<div className="activity-item-row-1">
					...
					>
						{getMethodName(history.method.toUpperCase())}
					</label>
					<label className="activity-url">
						{history.name.replace(/^https?:\/\//, "")}
					</label>
				</div>
				<div className="activity-item-row-2">
					...
					>
						...
						<DropdownPortal id={history.id} open={index === currentIndex}>
							<button
								onClick={(e) => onClickNewTab(e, history.id, history.name)}
							>
								Open in New Tab
							</button>
							<button onClick={(e) => onRunClick(e, history.id, history.name)}>
								Run Request
							</button>
							<div className="divider"></div>
							<button onClick={(e) => onSaveToCollection(e, history.id)}>
								Save to Collection
							</button>
							<button onClick={(e) => onRename(e, history.id)}>Rename</button>
							<button onClick={(e) => onDelete(e, history.id, history.name)}>
								Delete
							</button>
						</DropdownPortal>
					</div>
	...
	function handleClickOutside(evt: any) {
		...
		const menuEl = document.getElementById(
			"drop-down-menu-" + (history[refIndex.current]?.id ?? ""),
		);
		if (
			triggerEl &&
