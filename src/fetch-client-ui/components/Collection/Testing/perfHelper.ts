...
import { IPerfConfig, IPerfEndpointMetrics, IPerfMetrics, IPerfResultPoint } from "./perfTypes";
import { percentile } from "./perfEngine";

export function computeMetrics(results: IPerfResultPoint[], elapsedSec: number): IPerfMetrics {
	const total = results.length;
	const durations = results.map(r => r.duration).sort((a, b) => a - b);
	const failed = results.filter(r => r.isError || r.status >= 400 || r.status === 0).length;
...
export function computeEndpointBreakdown(results: IPerfResultPoint[], elapsedSec: number): IPerfEndpointMetrics[] {
	...
	grouped.forEach((points, requestId) => {
		...
		breakdown.push({ ...metrics, requestId, requestName: points[0].requestName, url: points[0].url, method: points[0].method });
	});
...
export function exportPerfCSV(
	...
	data += `${metrics.total},${metrics.success},${metrics.failed},${metrics.errorRate.toFixed(2)},` +
	...
	breakdown.forEach(b => {
		data += `${b.requestName},${b.total},${b.success},${b.failed},${b.errorRate.toFixed(2)},` +
