...
import {
	...
	HerdersValues,
	dataTypes,
} from "../../../../fetch-client-core/consts/headers.consts";
import {
	ITableData,
	TableType,
	TextType,
...
export const Table = (props: TableProps) => {
			...
			<>
				<td>
					<Autocomplete
						...
						onSelect={(val: string) =>
							index === data.length - 1
								? onRowAdd(val, index, true)
								: onRowUpdate(val, index, true)
						}
		...
		return (
			selectedVariable.id && (
				<TextEditor
					...
					onChange={(event) =>
						index === data.length - 1
							? onRowAdd(event, index, true)
							: onRowUpdate(event, index, true)
					}
					value={row.key}
					disabled={isEnabled(row, index) ? true : false}
					focus={false}
	...
	function getFileSelectColumn(row: ITableData, index: number) {
		return (
			<>
				<button className="file-select" onClick={(_e) => fileSelect(index)}>
					Select
				</button>
			...
			<>
				<td>
					{props.highlightNeeded
						? getKeyHighlightedColumn(row, index)
						: getKeyNonHighlightedColumn(row, index)}
				</td>
				<td>
					{props.type === "formData" && row.type === "File"
						? getFileSelectColumn(row, index)
						: props.highlightNeeded
							? getValueHighlightedColumn(row, index)
							: getValueNonHighlightedColumn(row, index)}
				</td>
			</>
		);
	}
		...
		return (
			<tr key={index}>
				...
				{props.type === "reqHeaders"
					? getRequetHeaderRow(row, index)
					: getKeyValueRow(row, index)}
