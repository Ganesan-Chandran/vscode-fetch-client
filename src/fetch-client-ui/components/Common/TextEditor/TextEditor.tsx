...
export const TextEditor = (props: TextEditorProps) => {
	...
	const matchedDecorated = useCallback((props: any) => {
		if (!checkSysVariable(props.decoratedText)) {
			let title = replaceDataWithVariable(
				props.decoratedText,
				refVarData.current,
			);
			if (title) {
	...
	function findWithRegex(words: string[], contentBlock: any, callback: any) {
		...
		const matches = [...text.matchAll(VAR_REGEX)].map((a) => ({
			index: a.index,
			word: a[0],
		}));
		const sysVarMatches = [...text.matchAll(SYS_VAR_REGEX)].map((a) => ({
			index: a.index,
			word: a[0],
		}));
		...
		sysVarMatches.forEach((match) => {
			if (
				SysVariables.includes(match.word) ||
				match.word.includes("{{#num,") ||
				match.word.includes("{{#date,")
			) {
				callback(match.index, match.index + match.word.length);
			}
		});
	...
	function findWithRegexUnMatched(
		words: string[],
		contentBlock: any,
		callback: any,
	) {
	...
	const matchedHandleStrategy = useCallback(
	...
	const unmatchedHandleStrategy = useCallback(
		(contentBlock: any, callback: any) => {
	...
	const decorator = useMemo(
		...
		[
			matchedHandleStrategy,
			unmatchedHandleStrategy,
			matchedDecorated,
			unmatchedDecorated,
		],
	);
	...
	useEffect(() => {
		if (props.value !== curValue) {
			...
			handleChange(
				EditorState.push(
					editorState,
					Modifier.replaceText(currentContent, selection, props.value),
