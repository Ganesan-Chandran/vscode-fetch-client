...
export const CurlProviderUI = (extensionUri: vscode.Uri) => {
	const disposable = vscode.commands.registerCommand(
		...
		() => {
			...
			curlPanel.webview.html = buildWebviewHtml(
				curlPanel.webview,
				extensionUri,
				"curlreq",
			);
			...
			curlPanel.webview.onDidReceiveMessage((reqData: any) => {
				if (reqData.type === requestTypes.runCurlRequest) {
					...
					if (!req || !req.url) {
						...
						curlPanel.webview.postMessage({
							type: responseTypes.runCurlResponse,
							request: null,
					...
					if (!req || !req.url) {
						curlPanel.webview.postMessage({
							type: responseTypes.curlErrorResponse,
					...
					AddToCollection(
						reqData.data.col,
						reqData.data.hasFolder,
						reqData.data.isNewFolder,
						curlPanel.webview,
