...
export const BulkExportProviderUI = (extensionUri: vscode.Uri) => {
	const disposable = vscode.commands.registerCommand(
		...
		(type: string) => {
			...
			bulkExportPanel.webview.html = buildWebviewHtml(
				bulkExportPanel.webview,
				extensionUri,
				`bulkexport@:@${type}`,
			);
						...
						.then((fileUri) => {
							if (fileUri && fileUri[0]) {
								bulkExportPanel.webview.postMessage({
									type: responseTypes.selectPathResponse,
