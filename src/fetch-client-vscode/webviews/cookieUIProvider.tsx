...
export const CookieUI = (extensionUri: vscode.Uri) => {
	const disposable = vscode.commands.registerCommand(
		...
		(id?: string) => {
			...
			cookiePanel.webview.html = buildWebviewHtml(
				cookiePanel.webview,
				extensionUri,
				`managecookies@:@${id}`,
			);
			...
			cookiePanel.webview.onDidReceiveMessage((reqData: any) => {
				if (reqData.type === requestTypes.getAllCookiesRequest) {
					...
					showConfirmationBox("Do you want to delete this cookie?").then(
						(data: any) => {
							if (data === "Yes") {
					...
					showConfirmationBox("Do you want to delete all cookies?").then(
						(data: any) => {
							if (data === "Yes") {
