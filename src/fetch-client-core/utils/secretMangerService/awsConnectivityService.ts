...
async function getRegionForProfile(
	profile: string,
): Promise<string | undefined> {
	if (regionCache.has(profile)) {
		return regionCache.get(profile);
	}
...
export async function getSecretCached(
	profile: string,
	secretName: string,
): Promise<{ ok: boolean; raw: string; cached: boolean }> {
...
export async function handleAwsCheckConnectivity(
	targets: TargetRef[],
): Promise<any[]> {
...
export async function handleAwsFetchAndCache(
	targets: TargetRef[],
): Promise<any[]> {
	const uniqueProfiles = [...new Set(targets.map((t) => t.profile))];
	const profileResults = await Promise.all(uniqueProfiles.map(checkProfile));
