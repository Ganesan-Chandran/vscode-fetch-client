...
export function findParentSettings(source: any, id: string, prevSettings: any = null): any | null {
...
export async function Col_Repository_DuplicateItem(
  ...
  historyId: string,
  folderType: boolean
): Promise<{ col: any; oldIds: string[]; ids: Record<string, string> } | 'copy-collection'> {
  const colDB = await getCollectionDB();
  const userCollections = colDB.getCollection('userCollections');
...
export async function Col_Repository_CopyToCollection(
  ...
  const clonedItems = cloneCollectionItems((sourceCol as ICollections).data, oldIds, ids);
...
export async function Col_Repository_UpdateCollectionItems(
  ...
  if (!colItem) {
    writeLog(`error::dbUpdateCollectionItems(): no collection found for id ${colId}`);
    return;
  }
  ...
  if (!target) {
    writeLog(`error::dbUpdateCollectionItems(): no folder found for id ${folderId}`);
    return;
  }
...
export async function Col_Repository_DeleteAllCollectionItems(colId: string, folderId: string): Promise<void> {
  const colDB = await getCollectionDB();
  const col = colDB.getCollection('userCollections').by('id', colId);
...
export async function Col_Repository_GetCollectionsByVariable(varId: string): Promise<string[]> {
  const colDB = await getCollectionDB();
...
export async function Col_Repository_GetVariableByColId(colId: string): Promise<string> {
  const colDB = await getCollectionDB();
  const colItem = colDB.getCollection('userCollections').by("id", colId);
...
export async function Col_Repository_GetParentSettings(colId: string, folderId: string): Promise<ISettings | null> {
  const colDB = await getCollectionDB();
  const colItem = colDB.getCollection('userCollections').by("id", colId);
...
export async function Col_Repository_ExecuteRequest(reqData: any, fetchConfig: FetchConfig): Promise<any> {
  ...
  return apiFetch(reqData.data.reqData, reqData.data.variableData, settings, null, fetchConfig);
}

export async function Col_Repository_ExecuteMultipleRequests(reqData: any, fetchConfig: FetchConfig): Promise<PromiseSettledResult<any>[]> {
  const colDB = await getCollectionDB();
  const colItem = colDB.getCollection('userCollections').by("id", reqData.data.colId);
