...
export async function Cookie_Repository_SaveCookie(item: ICookie): Promise<void> {
...
export async function Cookie_Repository_GetCookieById(id: string): Promise<ICookie[]> {
  const db = await loadDB();
  const userCookies = db.getCollection('userCookies');
  return userCookies.find({ id }) as ICookie[];
}

export async function Cookie_Repository_DeleteCookieById(id: string): Promise<void> {
  const db = await loadDB();
  const userCookies = db.getCollection('userCookies');
