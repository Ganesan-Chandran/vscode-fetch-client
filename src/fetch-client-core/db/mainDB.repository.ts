...
const {
	...
	saveDB: saveMainDB,
	flush: flushMainDB,
	invalidate: invalidateMainDB,
} = createAutoDBCache(mainDBPath);
...
function buildExportPayload(
	...
	const exportData: ExportPayload = {
		...
		id: col.id,
		name:
			col.name.toUpperCase().trim() === "DEFAULT" ? "Default Export" : col.name,
		version: "1.0",
		type: "collections",
...
export async function Main_Repository_SaveRequest(
...
export async function Main_Repository_UpdateRequest(
	reqData: IRequestModel,
): Promise<void> {
	try {
		const db = await getMainDB();
...
export async function Main_Repository_GetRequestItem(
	reqId: string,
...
export async function Main_Repository_GetExistingItem(
	id: string,
...
export async function Main_Repository_CopyExistingItems(
	oldIds: string[],
...
export async function Main_Repository_DeleteExistingItem(
	id: string,
): Promise<void> {
	try {
		const db = await getMainDB();
...
export async function Main_Repository_DeleteExistingItems(
	ids: string[],
): Promise<void> {
	if (ids.length === 0) {
		return;
...
export async function Main_Repository_RenameRequestItem(
	id: string,
	name: string,
): Promise<void> {
	try {
...
export async function Main_Repository_GetCollectionRequests(
	ids: string[],
): Promise<IRequestModel[]> {
	try {
		const db = await getMainDB();
...
export async function Main_Repository_BuildExport_V2(
	colId: string,
	hisId: string,
	folderId: string,
	key: string,
): Promise<IFetchClientExportV2> {
	const [db, colDB] = await Promise.all([getMainDB(), getCollectionDB()]);
	...
	const variables = variableId
		? await Var_Repository_FindById(variableId, false, key)
		: [];
	return ExportBuilderV2(
		cols[0],
		apiRequests,
		hisId,
		folderId,
		variables.length > 0 ? variables[0] : null,
	);
}

export async function Main_Repository_BuildExport_Postman(
	colId: string,
	hisId: string,
	folderId: string,
	key: string,
): Promise<PostmanSchema_2_1> {
	const [db, colDB] = await Promise.all([getMainDB(), getCollectionDB()]);
	const apiRequests = getRequestCollection(db);
	...
	const variables = variableId
		? await Var_Repository_FindById(variableId, false, key)
		: [];
	return ExportBuilderPostman2_1(
		cols[0] as ICollections,
		apiRequests,
		hisId,
		folderId,
		variables.length > 0 ? variables[0] : null,
...
export async function Main_Repository_BuildBulkExport(
	selectedCols: string[],
): Promise<ExportPayload[]> {
	const [db, colDB] = await Promise.all([getMainDB(), getCollectionDB()]);
	const apiRequests = getRequestCollection(db);
...
export async function Main_Repository_BuildBulkExportV2(
	selectedCols: string[],
	key: string,
	formatType: string,
): Promise<(IFetchClientExportV2 | PostmanSchema_2_1)[]> {
	const [db, colDB] = await Promise.all([getMainDB(), getCollectionDB()]);
	const apiRequests = getRequestCollection(db);
...
async function importFC(
	...
	const convertedData =
		verison === 1
			? fetchClientImporter(parsedData)
			: fetchClientV2Importer(parsedData);
	if (!convertedData?.fcCollection || !convertedData?.fcRequests) {
		throw new Error("Fetch Client import produced incomplete data.");
	}
...
export async function Main_Repository_Import(
	data: string,
	key: string,
): Promise<ImportResult> {
	...
	switch (type) {
		case ImportType.FetchClient_1_0:
			return importFC(data, 1, key);
		case ImportType.FetchClient_2_0:
			return importFC(data, 2, key);
		...
		case ImportType.Insomnia_4_5:
			return importInsomnia(data, key);
		default:
