...
import { ICollections, IFolder, IHistory, IVariable } from "../../../types/sidebar.types";
...
import {
  PostmanSchema_2_1, POSTMAN_SCHEMA_V2_1, Auth as PostmanAuth, ApikeyElement,
  Items, RequestObject, Body as PostmanBody, Header as PostmanHeader,
  URLObject, QueryParam, FormParameter, FormParameterType, Mode, URLEncodedParameter,
  AuthType, Event as PostmanEvent, Variable, VariableType
  ...
  if (hisId) {
    ...
    if (requests.length > 0) {
      if (folderId) {
        ...
        rootItems = [mapRequest(requests[0], mergeHeaders([], col.settings?.headers))];
      }
    }
  } else if (folderId) {
    const folder = findItem(col, folderId) as IFolder;
    ...
    const collectionHeaders = mergeHeaders([], col.settings?.headers);
    rootItems = rootData.map((item) => buildItem(item, apiRequests, collectionHeaders));
  }
...
function buildItem(
  ...
  if (requests.length === 0) {
    return {
      ...
      request: { method: (item.method ?? "GET").toUpperCase(), url: item.url ?? "" },
    };
  }
...
function buildFolderItem(
  ...
  folderItem.item = children.map((child) => buildItem(child, apiRequests, ownHeaders));
  return folderItem;
}
...
function mapRequest(req: IRequestModel, inheritedHeaders: ITableData[]): Items {
  ...
  const request: RequestObject = {
    ...
    header: mapHeaders(mergedHeaders, auth.injectedHeader ?? bearerHeaderOverride, contentTypeHeader),
    url,
    ...(body && { body }),
  };
...
function mapAuthPostman(
  ...
  switch (auth.authType) {
      ...
      if (prefix === "" || prefix.toLowerCase() === "bearer") {
        return {
          auth: {
            ...
            bearer: [{ key: "token", value: auth.password ?? "", type: "string" }],
          },
        };
      }
    ...
    case "aws": {
      const aws = auth.aws;
      if (!aws) { return { auth: { type: AuthType.Noauth } }; }
      ...
      if (aws.sessionToken) {
        awsv4.push({ key: "sessionToken", value: aws.sessionToken, type: "string" });
      }
    ...
    case "oauth2": {
      const o = auth.oauth;
      if (!o) { return { auth: { type: AuthType.Noauth } }; }
      ...
      if (o.advancedOpt?.audience) {
        oauth2.push({ key: "audience", value: o.advancedOpt.audience, type: "string" });
      }
      if (o.advancedOpt?.resource) {
        oauth2.push({ key: "resource", value: o.advancedOpt.resource, type: "string" });
      }
...
function mapBody(body?: IBodyData): { body?: PostmanBody; bearerHeaderOverride?: PostmanHeader; contentTypeHeader?: PostmanHeader; } {
 ...
 */
function mergeHeaders(parent: ITableData[], child?: ITableData[]): ITableData[] {
  const childKeys = new Set((child ?? []).map((h) => h.key.trim().toLowerCase()).filter(Boolean));
  const inheritedOnly = parent.filter((h) => !childKeys.has(h.key.trim().toLowerCase()));
  return [...inheritedOnly, ...(child ?? [])];
}

function mapHeaders(rows: ITableData[], ...extras: (PostmanHeader | undefined)[]): PostmanHeader[] {
 ...
 */
function mapUrlPostman(rawUrl: string, params: ITableData[]): { url: URLObject; extraQueryFromParams: boolean } {
  if (!rawUrl.trim()) {
  ...
  const hostPart = firstSlash === -1 ? hostAndPath : hostAndPath.slice(0, firstSlash);
  ...
  const variables = path.filter(p => p.startsWith(":")).map(p => ({ key: p.substring(1) }));
  ...
  const rebuiltQueryString = mergedQuery.length > 0
    ? "?" + mergedQuery.filter((q) => q.disabled !== true).map((q) => `${q.key}=${q.value}`).join("&")
    : "";
...
function mergeQueryParams(fromUrl: QueryParam[], fromParams: QueryParam[]): QueryParam[] {
  const paramKeys = new Set(fromParams.map((q) => (q.key ?? "").toLowerCase()));
  const urlOnly = fromUrl.filter((q) => !paramKeys.has((q.key ?? "").toLowerCase()));
  return [...urlOnly, ...fromParams];
}

function splitOnce(str: string, sep: string): [string, string | undefined] {
  const idx = str.indexOf(sep);
  return idx === -1 ? [str, undefined] : [str.slice(0, idx), str.slice(idx + 1)];
}
...
function getTestTitle(test: ITest): string {
  ...
  switch (test.parameter) {
    ...
    case "Response Body":
      ...
      if (test.action === "regex") { return "Response body matches regex"; }
...
function comparisonTest(title: string, target: string, expected: string, action: string): string[] {
