...
export class PostmanScriptParser {
	...
	getId(item: Items): IdEntry {
		...
		if (!entry) {
			throw new Error(
				"PostmanScriptParser: id requested for an item not seen during precomputeIds()",
			);
		}
		return entry;
	}
	...
	private buildUrlKeys(
		rawUrl: string,
		method: string,
	): { exactKey: string; looseKey: string } {
	...
	private normalizeUrl(rawUrl: string): { full: string; pathAndQuery: string } {
		...
		const pathAndQuery =
			...
			(sortedQuery ? `?${sortedQuery.toLowerCase()}` : "");
	...
	private buildJsonAssertionRules(
		varNames: string[],
	): Array<{ regex: RegExp; build: (m: RegExpMatchArray) => ITest | null }> {
		const varPattern = varNames
			.map((v) => v.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
			.join("|");
		...
		return [
			{
				regex: new RegExp(
					`pm\\.expect\\(\\s*${pathExpr}\\s*\\)\\.to\\.(eql|equal|include|contain)\\(\\s*(.+?)\\s*\\)\\s*;?`,
					"g",
				),
				build: (m) => ({
					parameter: "JSON",
			...
			{
				regex: new RegExp(
					`pm\\.expect\\(\\s*${pathExpr}\\s*\\)\\.to\\.be\\.(true|false)\\b`,
					"g",
				),
				build: (m) => ({
					parameter: "JSON",
			...
			{
				regex: new RegExp(
					`pm\\.expect\\(\\s*${pathExpr}\\s*\\)\\.to\\.be\\.null\\b`,
					"g",
				),
				build: (m) => ({
					parameter: "JSON",
			...
			{
				regex: new RegExp(
					`pm\\.expect\\(\\s*${pathExpr}\\s*\\)\\.to\\.exist\\b`,
					"g",
				),
				build: (m) => ({
					parameter: "JSON",
			...
			{
				regex: new RegExp(
					`pm\\.expect\\(\\s*${pathExpr}\\.length\\s*\\)\\.to\\.(eql|equal)\\(\\s*(\\d+)\\s*\\)`,
					"g",
				),
				build: (m) => ({
					parameter: "JSON",
			...
			{
				regex: new RegExp(
					`pm\\.expect\\(\\s*${pathExpr}\\s*\\)\\.to\\.be\\.at\\.(least|most)\\(\\s*(.+?)\\s*\\)`,
					"g",
				),
				build: (m) => ({
					parameter: "JSON",
			...
			{
				regex:
					/pm\.expect\(\s*function\s*\(\)\s*\{\s*pm\.response\.json\(\);\s*\}\s*\)\.to\.not\.throw\(\s*\)/g,
				build: () => ({
	...
	private readonly TEST_RULES: Array<{
		regex: RegExp;
		build: (m: RegExpMatchArray) => ITest | null;
	}> = [
		{
		...
		{
			regex:
				/pm\.expect\(\s*pm\.response\.code\s*\)\.to\.(eql|equal|not\.eql)\(\s*(\d+)\s*\)/g,
			build: (m) => ({
				parameter: "Response Code",
				action: m[1] === "not.eql" ? "notEqual" : "equal",
				expectedValue: m[2],
			}),
		},
		{
			regex:
				/pm\.expect\(\s*pm\.response\.code\s*\)\.to\.be\.at\.(most|least)\(\s*(\d+)\s*\)/g,
			build: (m) => ({
				parameter: "Response Code",
				action: m[1] === "most" ? "<=" : ">=",
				expectedValue: m[2],
		...
		{
			regex:
				/pm\.expect\(\s*pm\.response\.responseTime\s*\)\.to\.be\.(below|above)\(\s*(\d+)\s*\)/g,
			build: (m) => ({
		...
		{
			regex:
				/pm\.response\.to\.have\.header\(\s*["'`]([^"'`]+)["'`]\s*(?:,\s*["'`]([^"'`]+)["'`])?\s*\)/g,
			build: (m) => {
				...
				return {
					parameter: "Header",
					action: headerValue ? "equal" : "contains",
					expectedValue: headerValue ?? "",
					customParameter: headerName,
				};
			},
		},
		{
			...
			regex:
				/pm\.expect\(\s*pm\.response\.headers\.get\(\s*["'`]([^"'`]+)["'`]\s*\)\s*\)\.to\.(eql|equal|include)\(\s*["'`]([^"'`]*)["'`]\s*\)/g,
			build: (m) => ({
				parameter: "Header",
				action: m[2] === "include" ? "contains" : "equal",
				expectedValue: m[3],
				customParameter: m[1],
			}),
		},
		{
			regex:
				/pm\.expect\(\s*pm\.response\.headers\.get\(\s*["'`]([^"'`]+)["'`]\s*\)\s*\)\.to\.not\.eql\(\s*["'`]([^"'`]*)["'`]\s*\)/g,
			build: (m) => ({
				parameter: "Header",
				action: "notEqual",
				expectedValue: m[2],
		...
		},
		{
			...
			regex:
				/pm\.expect\(\s*pm\.response\.text\(\)\s*\)\.to\.include\(\s*["'`]([^"'`]*)["'`]\s*\)/g,
			build: (m) => ({
				parameter: "Response Body",
				action: "contains",
				expectedValue: m[1],
		...
		},
		{
			...
			regex:
				/pm\.expect\(\s*pm\.response\.text\(\)\s*\)\.to\.(eql|equal)\(\s*["'`]([^"'`]*)["'`]\s*\)/g,
			build: (m) => ({
				parameter: "Response Body",
				action: "equal",
				expectedValue: m[2],
	...
	private buildStatusAliasTests(script: string): ITest[] {
		const tests: ITest[] = [];
		const regex =
			/pm\.response\.to\.be\.(success|error|clientError|serverError|info|redirection)\b/g;
		let match: RegExpExecArray | null;
		while ((match = regex.exec(script)) !== null) {
			...
			if (range) {
				tests.push({
					parameter: "Response Code",
					action: ">=",
					expectedValue: String(range[0]),
				});
				tests.push({
					parameter: "Response Code",
					action: "<=",
					expectedValue: String(range[1]),
				});
			}
		}
		return tests;
	...
	parsePostmanTests(rawScript: string): ITest[] {
		...
		const rules = [
			...this.TEST_RULES,
			...this.buildJsonAssertionRules(varNames),
		];
	...
	private detectLiteral(valueExpr: string): {
		isLiteral: boolean;
		value: string;
	} {
	...
	private resolveSetVarSource(valueExpr: string): {
		parameter: string;
		key: string;
	} {
		const trimmed = valueExpr.trim();
	...
	parsePostmanSetVars(rawScript: string): ISetVar[] {
		...
		const rules = [
			/pm\.collectionVariables\.set\(\s*["'`]([^"'`]+)["'`]\s*,\s*(.+?)\)\s*;?$/gm,
			/pm\.globals\.set\(\s*["'`]([^"'`]+)["'`]\s*,\s*(.+?)\)\s*;?$/gm,
		];
	...
	parsePreRequestScript(rawScript: string): PreRequestParseResult {
		...
		while ((match = callStartRegex.exec(script)) !== null) {
			...
			if (stringUrlMatch) {
				...
				const objMethodMatch = argsText.match(
					/method\s*:\s*["'`]([^"'`]+)["'`]/,
				);
				if (objUrlMatch) {
			...
			if (targetUrl) {
				...
				if (candidate) {
					runRequests.push({
						...
						order: order++,
						condition: [
							{ parameter: "noCondition", action: "", expectedValue: "" },
						],
					});
