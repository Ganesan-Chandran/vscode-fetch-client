...
function safeSerialise(value: unknown, indent = 2): string {
	try {
		...
		if (s.length > MAX_EXAMPLE_SIZE) {
			writeLog(
				`warn::openApiImporter – generated example exceeds ${MAX_EXAMPLE_SIZE} bytes; omitted`,
			);
			return "";
		}
		return s;
...
class RefResolver {
	...
	resolve<T>(
		...
		if (this.cache.has(ref)) {
			return (this.cache.get(ref) as T | null) ?? null;
		}
...
class SchemaExample {
	...
	scalar(schema: OASchema | null | undefined, paramExample?: unknown): string {
		if (paramExample !== undefined && paramExample !== null) {
			return toStr(paramExample);
		}
		if (!schema) {
			return "";
		}
		...
		if (schema.example !== undefined && schema.example !== null) {
			return toStr(schema.example);
		}
		if (schema.default !== undefined && schema.default !== null) {
			return toStr(schema.default);
		}

		// x-enumNames: human-readable labels paired with enum values
		const enumNames = (schema as Record<string, unknown>)["x-enumNames"] as
			| string[]
			| undefined;
		if (enumNames?.length) {
			return enumNames[0];
		...
		if (schema.enum?.length) {
			return toStr(schema.enum[0]);
		}

		// OpenAPI 3.1: const keyword
		if ((schema as OASchema).const !== undefined) {
			return toStr((schema as OASchema).const);
		}
	...
	buildJson(raw: MaybeRef<OASchema>, depth = 0): unknown {
		...
		if (this.memo.has(schema)) {
			return this.memo.get(schema);
		}
	...
	private generateJson(schema: OASchema, depth: number): unknown {
		...
		if (schema.oneOf?.length) {
			return this.buildJson(
				this.resolveDiscriminator(schema, schema.oneOf),
				depth + 1,
			);
		}
		...
		switch (type) {
			...
			case "string":
				return (
					schema.enum?.[0] ?? schema.default ?? (schema.nullable ? null : "")
				);
			case "integer":
			case "number":
				return (
					schema.enum?.[0] ?? schema.default ?? (schema.nullable ? null : 0)
				);
			case "boolean":
				return schema.default ?? (schema.nullable ? null : true);
...
function mapApiKeyScheme(
	scheme: OASecurityScheme,
	_scopeList: string[],
): IAuth {
	const auth = deepClone(InitialAuth);
	...
	if (scheme.in === "cookie") {
		...
		writeLog(
			"info::SecurityMapper – apiKey cookie placement mapped to header named 'Cookie'",
		);
	} else {
...
function mapOAuth2Scheme(scheme: OASecurityScheme, scopeList: string[]): IAuth {
	...
	const scope =
		scopeList.join(" ") ||
		Object.keys(
			flows.clientCredentials?.scopes ?? flows.password?.scopes ?? {},
		).join(" ");
	...
	build(operationSecurity?: OASecurityRequirement[]): IAuth {
		...
		for (const req of requirements) {
			for (const [name, scopeList] of Object.entries(req)) {
				...
				if (!mapper) {
					writeLog(
						`info::SecurityMapper – unsupported type "${scheme.type}" → noauth`,
					);
					continue;
				}
...
function buildUrlencodedBody(
	...
	ex: SchemaExample,
	refs: RefResolver,
	_preferred: string | undefined,
): IBodyData {
	const body = deepClone(InitialBody);
	body.bodyType = "formurlencoded";
	body.urlencoded = [
		...schemaToRows(mediaType.schema, ex, refs),
		{ ...EMPTY_ROW },
	];
	return body;
}
...
function buildFormDataBody(
	...
	body.formdata = [
		...schemaToRows(mediaType.schema, ex, refs).map((r) => ({
			...r,
			type: "Text" as const,
		})),
...
function buildGraphqlBody(
	...
	if (mediaType.example !== undefined && mediaType.example !== null) {
		...
		if (schema) {
			query = safeSerialise(ex.buildJson(schema));
		}
	}
...
class BodyBuilder {
	...
	build(rawBody: MaybeRef<OARequestBody> | undefined): IBodyData {
		...
		const chosenRaw =
			BODY_MIME_PRIORITY.find((ct) => contentTypes.includes(ct)) ??
			contentTypes[0];
		if (!chosenRaw) {
			return empty;
		}
	...
	private buildRaw(
		...
		if (mediaType.example !== undefined && mediaType.example !== null) {
			...
			const schema = this.refs.deref<OASchema>(
				mediaType.schema as MaybeRef<OASchema>,
			);
			if (schema) {
				data = isXml
	...
	build(
		...
		for (const p of paramMap.values()) {
			if (p.in === "path" && p.required && !templateParams.has(p.name)) {
				writeLog(
					`warn::ParamBuilder – required path param "{${p.name}}" not found in "${path}"`,
				);
			}
		}
		...
		for (const p of paramMap.values()) {
			const schema = this.refs.deref<OASchema>(
				(p.schema as MaybeRef<OASchema>) ?? null,
			);
...
class ResponseNotesBuilder {
	...
	private static readonly INTERESTING = new Set([
		"200",
		"201",
		"204",
		"400",
	...
	build(
		...
		for (const [status, rawResp] of Object.entries(responses)) {
			if (!ResponseNotesBuilder.INTERESTING.has(status)) {
				continue;
			}
	...
	private extractResponseExample(resp: OAResponse): string {
		...
		if (media.schema) {
			const schema = this.refs.deref<OASchema>(
				media.schema as MaybeRef<OASchema>,
			);
			if (schema) {
	...
	build(): IVariable | null {
		...
		for (const server of this.doc.servers ?? []) {
			for (const [name, def] of Object.entries(server.variables ?? {})) {
				if (!name || BLOCKED_KEYS.has(name) || seen.has(name)) {
					continue;
				}
				seen.add(name);
				rows.push({
	...
	build(
		...
		for (const [path, pathItem] of Object.entries(paths)) {
			...
			for (const method of HTTP_METHODS) {
				...
				const { history, model } = requestBuilder(
					path,
					method,
					operation,
					pathItem,
		...
		const undeclared = [...tagBuckets.keys()].filter(
...
function resolveServerUrl(
	...
	const server =
		operation.servers?.[0] ?? pathItem.servers?.[0] ?? doc.servers?.[0];
...
export class OpenApiImport {
	...
	importCollection(): OpenApiImportResult {
		...
		const data = this.collection.build(
			(path, method, op, pathItem) =>
				this.buildRequest(path, method, op, pathItem),
			requests,
		);
...
function isOpenApiV3Document(doc: unknown): doc is OADocument {
	...
	if (typeof d.openapi !== "string" || !/^3\.\d+\.\d+/.test(d.openapi)) {
		return false;
	}
	if (typeof d.info !== "object" || d.info === null) {
		return false;
	}
	if (typeof (d.info as Record<string, unknown>).title !== "string") {
		return false;
	}
	return true;
}
