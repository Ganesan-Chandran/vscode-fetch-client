...
export class InsomniaImport {
	...
	private importEnvironment(
		...
		const variableName =
			env.name === "Base Environment" || !env.name ? workspaceName : env.name;
	...
	private getHeaders(headers: InsomniaHeader[] | undefined): ITableData[] {
		const result: ITableData[] = (headers ?? [])
			...
			.map((h) => ({
				...
				value: sanitiseHeaderPart(normaliseTplVars(h.value ?? "")),
			}));
	...
	private getAuthDetails(auth?: InsomniaAuthentication | null): IAuth {
		...
		switch (type) {
			...
			case "apikey": {
				...
				fcAuth.addTo = auth.addTo === "queryParams" ? "queryparams" : "header";
			...
			case "oauth2": {
				...
				fcAuth.oauth!.clientId = normaliseTplVars(auth.clientId ?? "");

				fcAuth.oauth!.clientSecret = normaliseTplVars(auth.clientSecret ?? "");

				fcAuth.oauth!.tokenUrl = normaliseTplVars(auth.accessTokenUrl ?? "");

				fcAuth.oauth!.scope = normaliseTplVars(auth.scope ?? "");

				fcAuth.oauth!.username = normaliseTplVars(auth.username ?? "");

				fcAuth.oauth!.password = normaliseTplVars(auth.password ?? "");

				fcAuth.oauth!.clientAuth = auth.credentialsInBody
					? ClientAuth.Body
					: ClientAuth.Header;

				fcAuth.oauth!.advancedOpt.audience = normaliseTplVars(
					auth.audience ?? "",
				);

				fcAuth.oauth!.advancedOpt.resource = normaliseTplVars(
					auth.resource ?? "",
				);
			...
			case "awsv4": {
				...
				fcAuth.aws.accessKey = normaliseTplVars(auth.accessKeyId ?? "");

				fcAuth.aws.secretAccessKey = normaliseTplVars(
					auth.secretAccessKey ?? "",
				);

				fcAuth.aws.region = normaliseTplVars(auth.region ?? "");

				fcAuth.aws.service = normaliseTplVars(auth.service ?? "");

				fcAuth.aws.sessionToken = normaliseTplVars(auth.sessionToken ?? "");
		...
		switch (mime) {
			...
			case "application/json": {
				...
				if (mime === "application/json") {
					try {
						const parsed = JSON.parse(body.text ?? "{}") as Record<
							string,
							unknown
						>;
	...
	private buildRequest(resource: InsomniaRequest): {
		history: IHistory;
		model: IRequestModel;
	} {
		const id = uuidv4();
	...
	private buildTree(
		...
		for (const child of children) {
			...
			switch (child._type) {
				case "request": {
					const { history, model } = this.buildRequest(
						child as InsomniaRequest,
					);
					requests.push(model);
					result.push(history);
					break;
	 ...
	 */
	private resolveBaseEnvironment(
		workspaceId: string,
	): InsomniaEnvironment | undefined {
		...
		return directEnvs.find((e) => !e.isPrivate) ?? directEnvs[0];
	}
